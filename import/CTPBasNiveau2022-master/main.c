...
int taille_chaine_de_caracteres(char *chaine)
{
    ...
    { // *(chaine + taille) -> permet de récupérer le contenu de la case taille du tableau chaine dynamiquement
        taille++;
    }
    return taille;
}
...
void affiche_tableau(int *tableau, int taille)
{
    for (int i = 0; i < taille; i++)
    {
        printf("%d", tableau[i]);
        if (i < taille - 1)
        { // Si on est pas à la dernière case du tableau on affiche un / pour séparer les cases
            printf(" / ");
        }
    }
    printf("\n");
